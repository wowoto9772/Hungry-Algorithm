...
int main() {
	...
	while (t--) {
		...
		for (int i = 1; i <= m; i++) { // vertex i's color
			int a, b;
			scanf("%d %d", &a, &b);
			lnk[a].push_back(b);
			lnk[b].push_back(a);
		...
		for (int i = 1; i <= n; i++) { // vertex i's coin's color

			for (int j = 1; j <= n; j++)cst[i][j] = INT_MAX;
		...
		pair <int, int> ans = mcmf.solve(src, snk);
