...
int main() {
	...
	MinCostFlow mcmf(snk+1), Mcmf(snk+1);
	...
	for (int i = 1; i <= n; i++) {
		for (int j = 1; j <= n; j++) {
			if (s[i] < a[j]) {
				mcmf.addEdge(i, n + j, pay(a[j]-s[i], lim), 1);
				Mcmf.addEdge(i, n + j, -pay(a[j] - s[i], lim), 1);
			}
		}
	}

	pair <int, int> l = mcmf.solve(src, snk);
	pair <int, int> r = Mcmf.solve(src, snk);
