...
int main(){
    ...
    int n = 0;
    while(++n && scanf("%d %d", &x[n], &y[n]) == 2);

    MinCostFlow mcmf(n+4);

    int src = 0, snk = n+3;
    int black = n+1, white = n+2;

    for(int i=1; i<=n; i++){
        mcmf.addEdge(src, i, 0, 1);
        mcmf.addEdge(i, white, -x[i], 1);
        mcmf.addEdge(i, black, -y[i], 1);
    }

    mcmf.addEdge(white, snk, 0, 15);
    mcmf.addEdge(black, snk, 0, 15);

    pair <int,int> ans = mcmf.solve(src, snk, 30);
