...
int mp[5][5] = { 10, 8, 7, 5, 1, 8 ,6, 4, 3, 1, 7, 4, 3, 2, 1, 5, 3, 2, 2, 1, 1, 1, 1, 1, 0 };
int ke['F' + 3];

int main() {
	int MAXCOST = 1000;

	ke['A'] = 0, ke['B'] = 1, ke['C'] = 2, ke['D'] = 3, ke['F'] = 4;
	...
	MinCostFlow mf((r*c) + 2);

	for (int i = 1; i <= r; i++) {
		for (int j = 1; j <= c; j++) {
			int k = (i & 1) ^ (j & 1);
			if (k) {
				mf.addEdge(src, node(i, j), 0, 1);
				for (int a = 0; a < 4; a++) {
					int ni = i + dr[a], nj = j + dc[a];
					if (ni < 1 || nj < 1 || ni > r || nj > c)continue;
					int cst = -mp[ke[str[i][j]]][ke[str[ni][nj]]];
					mf.addEdge(node(i, j), node(ni, nj), cst, 1);
				}
			}
			else {
				mf.addEdge(node(i, j), snk, 0, 1);
			}
		}
	}

	pair <int, int> ans = mf.solve(src, snk, 0);
