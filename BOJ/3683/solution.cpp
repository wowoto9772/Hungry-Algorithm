...
int main() {
	...
	while (t--) {
		...
		vector <int> clove, dlove, chate, dhate;
		...
		for (int i = 1; i <= m; i++) {
			...
			if (a == 'C') {
				// cat lovers
				clove.push_back(x);
				chate.push_back(y); // hate dog
			}
			else {
				// dog lovers
				dlove.push_back(x);
				dhate.push_back(y); // hate cat
			}

		}

		MaxFlowDinic mf;
		mf.Init(clove.size() + dlove.size() + 3);

		int src = clove.size() + dlove.size() + 1;
		int snk = src + 1;

		for (int i = 0; i < clove.size(); i++) {
			mf.AddEdge(src, i, 1);
			for (int j = 0; j < dlove.size(); j++) {
				if(!i)mf.AddEdge(clove.size() + j, snk, 1);
				if (clove[i] == dhate[j] ||
					chate[i] == dlove[j]) {
					// invalid
					mf.AddEdge(i, clove.size() + j, 1);
				}
			}
		}
