...
int main(){
    ...
    int src = 0, snk = 2*n+1, rsrc = 2*n+2;

    while(m--){
        int a, b;
        scanf("%d %d", &a, &b);
        mcmf.addEdge(n+a, b, -1, 1);
        mcmf.addEdge(n+b, a, -1, 1);
    }

    if(n&1)printf("0\n");
    else{

        mcmf.addEdge(src, rsrc, 0, 1);
        for(int i=1; i<=n; i++){
            mcmf.addEdge(rsrc, i, 0, 1);
            mcmf.addEdge(i, n+i, 0, 1);
            mcmf.addEdge(n+i, snk, 0, 1);
        }

        pair <int,int> ans = mcmf.solve(src, snk, 0);
