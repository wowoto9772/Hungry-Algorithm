...
int main(){
    ...
    for(int i=1; i<=n; i++)for(int j=1; j<=n; j++)c[i][j] = INT_MAX;
    ...
    for(int i=1; i<=n; i++){
        mcmf.addEdge(src, i, 0, 0);
        for(int j=1; j<=n; j++){
            if(c[i][j] == INT_MAX)continue;
            mcmf.addEdge(i, n+j, c[i][j], INT_MAX);
        }
        mcmf.addEdge(n+i, snk, 0, 0);
    }

    for(int j=0; j<mcmf.graph[src].size(); j++){
        int v = mcmf.graph[src][j].target;
        r[src][v] = j;
    }
    for(int i=1; i<=n; i++){
        for(int j=0; j<mcmf.graph[n+i].size(); j++){
            int v = mcmf.graph[n+i][j].target;
            r[n+i][v] = j;
        }
    }
    ...
    while(q--){
        ...
        if(c[s][e] != INT_MAX){
            mcmf.graph[src][r[src][s]].residual_capacity++;
            mcmf.graph[src][r[src][s]].orig_capacity++;
            mcmf.graph[n+e][r[n+e][snk]].residual_capacity++;
            mcmf.graph[n+e][r[n+e][snk]].orig_capacity++;
        }

        pair <int,int> tmp = mcmf.solve(src, snk);
