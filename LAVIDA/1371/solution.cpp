...
int main(){
	...
	while (t--){
		int v, e;
		scanf("%d %d", &v, &e);
		MinCostFlow mcmf(v + v + 3);
		for (int i = 1; i <= e; i++){
			int a, b, c;
			scanf("%d %d %d", &a, &b, &c);
			mcmf.addEdge(v + b, a, c, 1);
			mcmf.addEdge(v + a, b, c, 1);
		}
		for (int i = 0; i < v; i++){
			if (i != 2)mcmf.addEdge(i, v + i, 0, 1);
		}

		mcmf.addEdge(2, v + 2, 0, 2); // link to source
		mcmf.addEdge(v, v + v, 0, 1); // link to sink
		mcmf.addEdge(v + 1, v + v, 0, 1); // link to sink
		printf("%d\n", mcmf.solve(2, v + v).first);
	}
}