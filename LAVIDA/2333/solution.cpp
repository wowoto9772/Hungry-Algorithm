...
int main(){
	...
	while (scanf("%d %d", &v, &e) == 2){
		MinCostFlow mcmf(2 * v + 2);
		for (int i = 1; i <= e; i++){
			int a, b, c;
			scanf("%d %d %d", &a, &b, &c);
			mcmf.addEdge(v + a, b, c, 1);
		}
		mcmf.addEdge(0, 1, 0, 2); // link to source
		mcmf.addEdge(1, 1 + v, 0, 2);
		mcmf.addEdge(v, v + v, 0, 2); // link to sink
		mcmf.addEdge(v + v, v + v + 1, 0, 2);
		for (int i = 2; i < v; i++)mcmf.addEdge(i, v + i, 0, 1);
		printf("%d\n", mcmf.solve(0, 2 * v + 1).first);
	}
}